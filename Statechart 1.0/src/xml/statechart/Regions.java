...
public class Regions {
	...
	public Hashtable<Vertices, String> constructSetCRec(int i, Vertices v, Hashtable<String, Vertices> hashId,
			ArrayList<Vertices> visitados, String p, Hashtable<Vertices, String> setCHash,
			Hashtable<Vertices, Set<String>> hashFilhos) {
		/* Adiciono o vertice na lista de visitados */
		visitados.add(v);

		/* Associo o caminho ate aqui com o vertice */
		setCHash.put(v, p); // where p is "path" but actually means transitions

		String newT = ""; // New Transitions
		...
		if (vEhPai) {
			for (int j = 0; j < v.getListRegions().size(); j++) {
				...
				/*
				 * Construo o conjunto de cobertura da regiao dos filhos
				 * separadamente
				 */
				...
				/*
				 * Renovo a associacao do caminho ate aqui com o vertice pai,
				 * deixando marcado que preciso expandir o caminho
				 */
				...
				/*
				 * Associo o conjunto dos caminhos dos filhos com o vertice pai.
				 * O hashFilhos eh usado no TestGenerator
				 */
		...
		for (OutgoingTransitions out : v.getListTransitions()) {
			...
			/*
			 * Se o vertice de destino ainda nao foi visitado e o vertice de
			 * destino está nessa regiao
			 */
			if (!visitados.contains(proxV) && this.getListVertices().contains(proxV)) {
				...
				if (vEhPai) {
					 ...
					 */
					constructSetCRec(i + 1, proxV, hashId, visitados, p + " " + "@" + v.getId() + "@ " + newT, setCHash,
							hashFilhos);
				} else {
